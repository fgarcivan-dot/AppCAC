    ...
    <div className={`relative overflow-hidden rounded-[2rem] border transition-all duration-1000 ${
      ...
      <div className="relative z-10 flex flex-col gap-3 p-4">
        ...
        <div className="flex justify-between items-center opacity-40">
           <span className="text-[7px] font-black tracking-[0.4em] uppercase">{title}</span>
        ...
        {/* Tactical HUD Grid */}
        <div className="grid grid-cols-3 divide-x divide-white/5 bg-black/20 rounded-2xl border border-white/5 py-3 shadow-inner">
           <div className="flex flex-col items-center">
              ...
              <span className="text-[7px] font-black tracking-widest text-white/30 uppercase">VITS.</span>
           </div>
           <div className="flex flex-col items-center">
              <span className={`text-xl font-black tracking-tighter tabular-nums ${theme === 'day' ? "text-slate-400" : "text-white/40"}`}>{draws}</span>
              <span className="text-[7px] font-black tracking-widest text-white/30 uppercase">EMPS.</span>
           </div>
           <div className="flex flex-col items-center">
              ...
              <span className="text-[7px] font-black tracking-widest text-white/30 uppercase">DERS.</span>
           </div>
        </div>

        {/* Digital Segmented Progress Bar */}
        <div className="flex items-center gap-3">
           <div className="flex-1 h-1.5 w-full overflow-hidden rounded-full bg-white/5 p-[1px] relative">
              <div className="absolute inset-0 flex">
                 {/* Segment separators to look "digital" */}
                 {[...Array(20)].map((_, i) => (
                    <div key={i} className="flex-1 border-r border-black/20 z-20" />
                 ))}
              </div>
           ...
           <span className={`text-[8px] font-black tabular-nums transition-colors duration-1000 ${theme === 'day' ? "text-slate-400" : "text-white/40"}`}>
