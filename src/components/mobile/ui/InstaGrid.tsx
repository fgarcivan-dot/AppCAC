...
export function InstaGrid({ data }: InstaGridProps) {
  ...
  return (
    <div className="w-full py-16 relative overflow-hidden transition-colors duration-1000 bg-white">
      
      {/* 🌊 Background Light Leaks */}
      <div className="absolute top-1/2 left-0 w-64 h-64 bg-primary/[0.03] rounded-full blur-[100px] pointer-events-none" />
        ...
        {finalImages.map((src, i) => {
          ...
          return (
            <div key={i} 
               ...
               } border-black/5 shadow-lg shadow-black/5`}>
      ...
      <div className="px-6 relative z-10 w-full mt-10">
        <a 
          ...
          className="flex items-center justify-center gap-5 w-full py-5 rounded-[2rem] font-black uppercase text-[10px] tracking-[0.5em] transition-all active:scale-[0.98] bg-primary text-white shadow-[0_10px_30px_rgba(218,41,28,0.2)] hover:scale-[1.02]"
        >
