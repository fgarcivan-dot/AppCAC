  ...
  return (
    <div className="flex flex-col gap-10 p-6 animate-in fade-in duration-700 bg-[#050505] text-white min-h-screen">
      ...
      <header className="flex items-center justify-between">
        <h1 className="text-4xl font-black tracking-tighter uppercase text-white">CONTACTO<br /><span className="text-primary tracking-norm">DO CLUB</span></h1>
      ...
      <section className="flex flex-col gap-4">
        {infoItems.map((item, i) => (
          <motion.div
            ...
            className="flex items-center gap-4 rounded-3xl p-5 border bg-white/5 border-white/10 active:bg-white/10"
          >
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/5 text-primary">
              <item.icon size={20} />
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] font-black tracking-[0.4em] uppercase text-white opacity-40">{item.title}</span>
              <span className="text-xs font-black uppercase text-white">{item.value}</span>
            </div>
          </motion.div>
        ))}
      </section>
      ...
      {/* Socials Card */}
      <section className="rounded-[2.5rem] p-8 border bg-white/[0.02] border-white/10 shadow-2xl">
        <div className="flex flex-col items-center gap-6">
             <h3 className="text-[10px] font-black tracking-[0.4em] uppercase text-white opacity-40">SÍGUENOS</h3>
            <div className="flex gap-8">
                {socials.map((social) => (
                    <motion.a
                        ...
                        whileTap={{ scale: 0.9 }}
                        className="flex h-14 w-14 items-center justify-center rounded-full border bg-white/5 border-white/10 text-white hover:text-primary hover:border-primary transition-all"
                    >
                        <social.icon size={24} />
                    </motion.a>
                ))}
      ...
      <div className="mt-10 flex flex-col items-center text-center">
          <span className="text-[8px] font-black tracking-[0.5em] uppercase text-white opacity-40">CLUB ATLÉTICO CERCEDENSE</span>
          <span className="mt-1 text-[8px] font-black uppercase tracking-widest leading-loose text-white opacity-10">
